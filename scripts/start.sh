#!/usr/bin/env bash
# SurplusClaim AI — launch all three Node services (dashboard:3000, website:4000, intake:5000).
# Usage: bash scripts/start.sh   (idempotent — safe to run again)
set -uo pipefail
...
start_one() {
  ...
  (cd "$dir" && setsid nohup node server.js >>"$log" 2>&1 < /dev/null & echo $! >"$LOG_DIR/$name.launch.pid")
