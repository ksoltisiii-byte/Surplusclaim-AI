...
function db() {
  if (!_db) {
    ...
    if (!paymentCols.includes('stripe_invoice_id')) {
      db().exec('ALTER TABLE payments ADD COLUMN stripe_invoice_id TEXT');
    }
    const claimCols = db().prepare('PRAGMA table_info(claims)').all().map((c) => c.name);
    if (!claimCols.includes('venue')) {
      db().exec('ALTER TABLE claims ADD COLUMN venue TEXT');
    }
    if (!claimCols.includes('confirmation_number')) {
      db().exec('ALTER TABLE claims ADD COLUMN confirmation_number TEXT');
    }
  }
  return _db;
}
